...
export async function enqueueM11Work(input: {
  ...
  const sql = getDatabase();
  const diagnostics = sql.json({
    resumeVersionId: input.resumeVersionId ?? null,
    opportunityId: input.opportunityId ?? null,
  } as never);
  const [row] = await sql`insert into public.work_items
    ...
    values (${input.workType}::public.work_type,'RESUME'::public.work_class,${input.userId}::uuid,${input.resumeVersionId ?? null}::uuid,${input.opportunityId ?? null}::uuid,${input.parserVersion ?? null},${input.algorithmVersion ?? null},${input.idempotencyFingerprint},${diagnostics})
