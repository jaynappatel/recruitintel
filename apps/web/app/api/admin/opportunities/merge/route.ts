...
import { mergeOpportunityRequestSchema, opportunitySchema } from "@recruitintel/types";
...
export async function POST(request: Request) {
  try {
    ...
    const parsed = mergeOpportunityRequestSchema.safeParse(body);
    if (!parsed.success) return validationError(parsed.error);
    const opportunity = await mergeOpportunities({ ...parsed.data, actorUserId: actor.user.id });
