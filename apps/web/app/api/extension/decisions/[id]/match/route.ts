...
import { addBrowserDecisionToMatch } from "@recruitintel/db";
import { browserDecisionMatchRequestSchema } from "@recruitintel/types";
...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const parsed = browserDecisionMatchRequestSchema.safeParse(
      await request.json().catch(() => null),
    );
    if (!parsed.success) return validationError(parsed.error);
    return NextResponse.json(
      {
        data: await addBrowserDecisionToMatch(actor.grant.userId, id, parsed.data.resumeVersionId),
      },
