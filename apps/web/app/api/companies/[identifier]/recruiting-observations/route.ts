...
import { getCompany, listPublicRecruitingObservations } from "@recruitintel/db";
import {
  publicObservationListQuerySchema,
...
export async function GET(request: Request, { params }: Context) {
  ...
  const parsed = publicObservationListQuerySchema.safeParse(
  ...
  try {
    ...
    const page = await listPublicRecruitingObservations(company.id, parsed.data);
    return NextResponse.json({
      data: page.items.map((item) => publicRecruitingObservationSchema.parse(item)),
