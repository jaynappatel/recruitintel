...
import { getSchool, listSchoolCompanies } from "@recruitintel/db";
import { listQuerySchema, schoolCompanyIntelligenceSchema } from "@recruitintel/types";
...
export async function GET(request: Request, { params }: Context) {
  ...
  try {
    ...
    const result = await listSchoolCompanies(school.id, parsed.data);
    return NextResponse.json({
      data: result.items.map((item) => schoolCompanyIntelligenceSchema.parse(item)),
