...
| 5 | RecruitingDate, CalendarItem, deterministic ApplicationPlan/Task, Google OAuth with state and PKCE, encrypted refresh credentials, connection/preferences, provider abstraction, one-way retry-safe Google sync, external mappings, sync request/run observability | Backend complete, UI not reconciled: `apps/web/lib/api/calendar.ts` still runs an in-memory mock and duplicates plan generation in the browser. All calendar routes resolve one configured MVP owner UUID rather than an authenticated user. |
